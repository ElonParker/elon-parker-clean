  ...
  return (
    <Layout isAuthenticated={true}>
      <div className="animate-fade-in">
        ...
        {/* ✅ ADMIN BANNER — Só aparece para admins */}
        {user?.role === 'admin' && (
          <Link
            href="/admin"
            className="block mb-8 bg-gradient-to-r from-red-600/20 to-orange-600/20 border border-red-500/50 rounded-2xl p-6 hover:border-red-400/70 transition group"
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-400 text-sm">Gerenciar agentes, tarefas, usuários e configurações do sistema</p>
                </div>
              </div>
              <div className="text-3xl text-slate-400 group-hover:text-white transition">→</div>
            </div>
          </Link>
        )}
        ...
        <div className="bg-darker/50 border border-gray-700 rounded-2xl p-8">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            ...
            {user?.role === 'admin' ? (
              <Link
                href="/admin/settings"
                className="bg-red-500/20 hover:bg-red-500/30 border border-red-500/50 text-white p-4 rounded-lg transition text-center"
              >
                🛡️ Admin Settings
              </Link>
            ) : (
